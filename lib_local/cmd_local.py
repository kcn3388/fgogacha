...
async def local_find_cmd(bot: HoshinoBot, ev: CQEvent):
    ...
    if len(msg) < 2:
        await bot.send(ev, "食用指南：[查询fgo纹章 + 纹章名字]")
        return
    ...
    async with ClientSession(headers=headers) as session:
        if len(cmd_data) == 0:
            ...
            for each in msg:
                ...
                name, stat = await lib_cmd_online(url, session)
                if stat == -100:
        ...
        if is_detail:
            ...
            for each in cmd_data:
                ...
                if os.path.exists(img_path):
                    ...
                    msg_send += "卡面：\n"
                    card = await gen_img_from_url(each["cards_url"], session)
            ...
            try:
                ...
                await bot.send(ev, "消息被风控，可能是消息太长，请尝试更精确指定纹章")
                return
            ...
            try:
                ...
                sv_lib.logger.error(f"转发群消息失败：{e}")
                await bot.send(ev, "消息被风控，可能是消息太长，请尝试更精确指定礼装")
                return
