...
async def get_all_cmd(session: ClientSession) -> Union[Exception, Tuple[int, Union[None, list]]]:
    root_cmd_url = "https://fgo.wiki/w/%E6%8C%87%E4%BB%A4%E7%BA%B9%E7%AB%A0%E5%9B%BE%E9%89%B4"
    try:
        raw_data = (await get_content(root_cmd_url, session)).decode()
    ...
    for i in range(0, len(data), 10):
        ...
        if cmd not in old_all_cmd:
            updated_commands.append(int(cmd["id"]))
        commands.append(cmd)
